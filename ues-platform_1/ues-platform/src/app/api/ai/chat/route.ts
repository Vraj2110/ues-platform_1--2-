...
import { fetchInstagramRecentMedia, fetchFacebookRecentPosts, fetchYouTubeRecentVideos } from "@/lib/server/oauth";
  ...
  try {
    ...
    if (!allPosts || allPosts.length === 0) {
      ...
      if (ytSecrets?.accessToken) {
        try {
          const ytVideos = await fetchYouTubeRecentVideos(ytSecrets.accessToken as string, ytSecrets.refreshToken as string, 25).catch(() => null);
    ...
    let currentPosts = allPosts.filter(p => new Date(p.publishedAt) >= currentCutoff);
